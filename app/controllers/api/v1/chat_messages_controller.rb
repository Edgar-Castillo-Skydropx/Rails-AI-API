class Api::V1::ChatMessagesController < ApplicationController
  ...
  def authorize_request
    if params[:token].blank?
      render json: { message: "Token is missing or invalid." }, status: 403 and return
    end

    @api_key = ApiKey.where(token: params[:token]).first
    if !@api_key
      render json: { message: "You must pass a token paremeter with a valid api key. If you need to generate an api key visit our website http://localhost:3000" }, status: 401
    end
  end
end
